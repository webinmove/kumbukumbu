...
module.exports.Kumbukumbu = class Kumbukumbu {
  ...
  async clearCache () {
    const keysToDelete = await this.options.redisClient.keys('CACHE:*');
    await asyncForEach(keysToDelete, async keyToDelete => {
      await this.options.redisClient.del(keyToDelete);
    });
